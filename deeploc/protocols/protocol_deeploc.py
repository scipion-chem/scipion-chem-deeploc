...
class ProtDeepLoc(EMProtocol):
  ...
  def createOutputStep(self):
      ...
      if self.inputType.get() == 0:
          ...
          self._defineOutputs(
              outputAtomStruct=model
          )

      # SetOfAtomStructs
      elif self.inputType.get() == 1:
          outputSet = SetOfAtomStructs.create(
              self._getPath()
          )
      ...
      # Single SequenceChem
          ...
          model._localizationPerc = String(
              str(outCsv[0])
          )
  ...
  def getResiduePredictions(self, proteinId, outPath):
      ...
      for filePath in glob.glob(
              os.path.join(outPath, 'alpha_*.csv')
      ):
  ...
  def _setResiduePredictions(self, model, residuePredictions):
      if not residuePredictions:
          return

      if isinstance(model, (SequenceChem, SetOfSequencesChem)):
  ...
  def _addDeepLocAttributes(self, model, residuePredictions):
      if not residuePredictions:
          return
